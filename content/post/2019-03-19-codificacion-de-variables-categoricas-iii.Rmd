...
hist <- model %>% fit(x = inputVariables, y = as.matrix(df$ScaledUsers), epochs = 70, batch_size = 2)
```
...
df <- df %>% left_join( embeddings_wday, by= c("weekDayF" = "lookup"))

df %>% select(weekDayF, X1, X2, X3) %>% head()
```
