...
K = 10 #number of subgroups to detect
subgroups=[]
score=[]
for k in range(0,K):
    CATE = []
...
CalcularCate <-  function(i){
  
  filtro_antes   = df_final[[variables_summarizar[i]]] == 1 & df_final$corte_fecha == "antes"
  filtro_despues = df_final[[variables_summarizar[i]]] == 1 & df_final$corte_fecha != "antes"
  
  media_antes   = mean(df_final$order_value[filtro_antes])
  media_despues = mean(df_final$order_value[filtro_despues])
...
variables_summarizar <- colnames(df_final)[3:ncol(df_final)]

tictoc::tic()
k = 10
res = map_dbl(1:length(variables_summarizar), CalcularCate)
names(res) <-  variables_summarizar
sort(res, decreasing = TRUE)[1:k]
tictoc::toc()

```

Y bueno, parece que en este caso, los viejos [R]ockeros no lo hacen mal del todo, de hecho bastante más rápido. 
